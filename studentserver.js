 ...
 * /students:
 *   post:
 *     summary: Creates a nsfsdew student object with all of its attributes.
 ...
 *     description: Use this endpoint to search for students based on their last name.
 *     parameters:
 *       - name: last_name
 *         description: Last name of the student to search for.
 *         in: path
 *         required: true
 *         schema:
 *           type: string
 ...
 *       404:
 *         description: Error. No students with the specified last name found.
 */
app.get('/students/search/:last_name', function (req, res) {
  ...
  glob("students/*.json", null, function (err, files) {
    ...
    files.forEach(function (file) {
      var studentData = fs.readFileSync(file, "utf8");
    ...
    if (matchingStudents.length === 0) {
      var rsp_obj = {};
      rsp_obj.last_name = searchLastName;
      rsp_obj.message = 'error - no students found with the specified last name';
      return res.status(404).send(rsp_obj);
    } else {
      var obj = {};
...
var listener = app.listen(port) //start server
